...
/obj/item/weapon/antag_spawner/contract
	name = "contract"
	desc = "A magic contract previously signed by an apprentice. In exchange for instruction in the magical arts, they are bound to answer my call for aid."
...
/obj/item/weapon/antag_spawner/contract/attack_self(mob/user as mob)
	...
	if(used)
		...
		dat += "<I>My apprentice is skilled in the ancient art of Clown Magic. They know The Clown Curse and Shoe Snatch</I><BR>"
		dat += "<A href='byond://?src=\ref[src];school=misdirection'>Misdirection</A><BR>"
		dat += "<I>My apprentice is skilled in misdirection and trickery. They know Subjugate and Blind</I><BR>"
		dat += "<A href='byond://?src=\ref[src];school=muscle'>Muscle Magic</A><BR>"
		dat += "<I>My apprentice is skilled in muscle based wizardry. They know Mutate and Blink</I><BR>"
		dat += "<A href='byond://?src=\ref[src];school=technology'>Technology</A><BR>"
		dat += "<I>My apprentice is skilled in technology and future magic. They know Disable Tech and Lightning</I><BR>"
	...
	if(loc == H || (in_range(src, H) && istype(loc, /turf)))
		...
		if(href_list["school"])
			...
			if(candidates.len)
				...
				to_chat(H, "Unable to reach my apprentice! You can either attack the spellbook with the contract to refund my points, or wait and try again later.")
...
/obj/item/weapon/antag_spawner/contract/spawn_antag(var/client/C, var/turf/T, var/type = "")
	...
	M.key = C.key
	to_chat(M, "<B>I am the [usr.real_name]'s apprentice! You are bound by magic contract to follow their orders and help them in accomplishing their goals.")
	switch(type)
		if("destruction")
			...
			to_chat(M, "<B>My service has not gone unrewarded, however. Studying under [usr.real_name], you have learned powerful, destructive spells. You are able to cast magic missile and fireball.")
		if("bluespace")
			M.add_spell(new /spell/area_teleport, iswizard = TRUE)
			M.add_spell(new /spell/targeted/ethereal_jaunt, iswizard = TRUE)
			to_chat(M, "<B>My service has not gone unrewarded, however. Studying under [usr.real_name], you have learned reality bending mobility spells. You are able to cast teleport and ethereal jaunt.")
		/*
		if("healing")
			...
			to_chat(M, "<B>My service has not gone unrewarded, however. Studying under [usr.real_name], you have learned livesaving survival spells. You are able to cast charge and forcewall.")
		*/
		if("robeless")
			M.add_spell(new /spell/aoe_turf/knock, iswizard = TRUE)
			M.add_spell(new /spell/targeted/mind_transfer, iswizard = TRUE)
			to_chat(M, "<B>My service has not gone unrewarded, however. Studying under [usr.real_name], you have learned stealthy, robeless spells. You are able to cast knock and mindswap.")
		if("clown")
			...
			to_chat(M, "<B>My service has not gone unrewarded, however. Studying under [usr.real_name], you have learned the venerable and ancient art of Clown Magic. You are able to cast the clown curse and shoe snatch.")
		if("misdirection")
			M.add_spell(new /spell/targeted/subjugation, iswizard = TRUE)
			M.add_spell(new /spell/targeted/genetic/blind, iswizard = TRUE)
			to_chat(M, "<B>My service has not gone unrewarded, however. Studying under [usr.real_name], you have learned spells for misdirection and trickery. You are able to cast subjugate and blind.")
		if("muscle")
			M.add_spell(new /spell/targeted/genetic/mutate, iswizard = TRUE)
			M.add_spell(new /spell/aoe_turf/blink, iswizard = TRUE)
			to_chat(M, "<B>My service has not gone unrewarded, however. Studying under [usr.real_name], you have gained great strength and a natty physique. You are able to cast mutate and blink.")
		if("technology")
			M.add_spell(new /spell/aoe_turf/disable_tech, iswizard = TRUE)
			M.add_spell(new /spell/lightning, iswizard = TRUE)
			to_chat(M, "<B>My service has not gone unrewarded, however. Studying under [usr.real_name], you have futuristic, technological spells. You are able to cast disable tech and lightning.")
...
/obj/item/weapon/antag_spawner/borg_tele/attack_self(mob/user as mob)
	...
	if(borg_candicates.len > 0)
		...
		to_chat(user, "<span class='notice'>Unable to connect to Syndicate Command. Please wait and try again later or use the teleporter on my uplink to get my points refunded.</span>")
