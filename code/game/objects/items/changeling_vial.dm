...
/obj/item/changeling_vial/attack_self(mob/user as mob)
	if(istype(user, /mob/living/carbon/human))
		...
		if(H.mind)
			...
			if(!ischangeling(H))
				user.visible_message("<span class='warning'>As [user] crushes the vial, a mass of black goo leaps at \his face!.</span>","<span class='warning'>As you try to remove the cap, you crush the vial in my hand!</span> <span class='danger'>A mass of black goo leaps at you from the vial!</span>")
				...
				H.sleeping += 10
				sleep(100)
				to_chat(H, "I feel my consciousness slipping away...")
				sleep(100)
				...
				to_chat(user, "<span class='notice'>I attempt to remove \the [src]'s cap, but the changeling inside it informs you of its presence. You decide to leave it be.</span>")
	else
		to_chat(user, "<span class='notice'>I try to remove \the [src]'s cap, but it won't budge.</span>")
