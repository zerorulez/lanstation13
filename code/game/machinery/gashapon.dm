...
/obj/machinery/gashapon/attackby(var/obj/O as obj, var/mob/user as mob)
	if (is_type_in_list(O, list(/obj/item/weapon/coin/, /obj/item/weapon/reagent_containers/food/snacks/chococoin)))
		if(user.drop_item(O, src))
			user.visible_message("<span class='notice'>[user] puts a coin into [src] and turns the knob.</span>", "<span class='notice'>I put a coin into [src] and turn the knob.</span>")
...
/obj/item/weapon/capsule/attack_self(mob/M as mob)
	...
	M.put_in_hands(I)
	I.add_fingerprint(M)
	to_chat(M, "<span class='notice'>I got \a [I]!</span>")
	qdel(src)
	return
