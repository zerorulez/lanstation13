...
/obj/item/weapon/subspacetunneler/attack_self(mob/user as mob)
	...
	user.put_in_hands(B)
	loaded_crystal = null
	to_chat(user, "I unload \the [B] from \the [src].")
...
/obj/item/weapon/subspacetunneler/verb/remove_matter_bin()
	...
	if(!loaded_matter_bin)
		...
		usr.put_in_hands(M)
		loaded_matter_bin = null
		to_chat(usr, "I remove \the [M] from \the [src].")
	update_verbs()
	update_icon()
...
/obj/item/weapon/subspacetunneler/proc/receive(obj/O as obj, mob/living/user as mob|obj, params, reflex = 0)
	...
	if(!loaded_matter_bin)
		if(!istype(O, /obj/item))
			...
			user.put_in_hands(I)
			user.visible_message("[user] pulls \the [I] to \himself through a subspace rift!","I pull \the [I] to yourself through a subspace rift.")
