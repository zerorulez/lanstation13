				...
				if(H.species.name != "Diona")
					...
					if(percent_machine>5)
						if(holder.has_reagent("mednanobots"))
							percent_machine-=1
							if(prob(20))
								to_chat(H, pick("I feel more like yourself again."))
					if(H.dizziness != 0)
...
/datum/reagent/comnanobots/on_mob_life(var/mob/living/M)
	...
	switch(volume)
		if(1 to 4.5)
			M.Jitter(5)
			if(prob(10))
				to_chat(M, "I feel slightly energized, but nothing happens")
...
/datum/reagent/irradiatedbeans
	...
	id = IRRADIATEDBEANS
	description = "I can almost taste the lead sheet behind it!"
	reagent_state = LIQUID
...
/datum/reagent/drink/sportdrink
	name = "Sport Drink"
	id = SPORTDRINK
	description = "I like sports, and you don't care who knows."
	sport = 5
