...
/obj/item/weapon/barricade_kit/attack_self(mob/user as mob)
	...
	if(istype(user.loc, /turf/space))
		to_chat(user, "<span class='warning'>I can't build barricades out in space.</span>")
		return
	...
	switch(alert("What do you want ([kit_uses] use\s left)", "Barricade Kit", "Directional", "Full Tile", "Cancel", null))
		if("Directional")
			...
			"<span class='notice'>I start building a barricade.</span>")
			if(do_after(user, src, 30))
				to_chat(user, "<span class='notice'>I finish the barricade.</span>")
		...
		if("Full Tile")
			...
			"<span class='notice'>I start building a full barricade.</span>")
			if(do_after(user, src, 50))
				to_chat(user, "<span class='notice'>I finish the full barricade.</span>")
...
/obj/item/weapon/barricade_kit/preattack(var/atom/A, mob/user as mob)
	if(istype(A, /obj/machinery/door/airlock) || istype(A, /obj/structure/window/full) || istype(A, /obj/structure/grille)) //Apply on a full window or an airlock
		...
		"<span class='notice'>I start barricading \the [A].</span>")
		if(do_after(user, T, 30))
			...
			"<span class='notice'>I barricade \the [A].</span>")
			kit_uses--
			if(kit_uses < 1)
