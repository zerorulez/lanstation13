...
/obj/machinery/wish_granter/attack_hand(var/mob/user as mob)
	...
	if(charges <= 0)
		...
		to_chat(user, "<span class='sinister'>I feel a dark stirring inside of \the [src], something you want nothing of! Your instincts are better than any man's.</span>")
		return
		...
		user.visible_message("<span class='sinister'>[user] touches [src] delicately, causing it to stir.</span>", "<span class='sinister'>My first touch makes [src] stir, listening to you. Are you still sure about this ?</span>")
		insisting++
		...
		spawn(10) //OH SHI-
			...
			user.visible_message("<span class='sinister'>[user] clenches in pain before \the [src] and then raises back up with a demonic and soulless expression!</span>", "<span class='sinister'>\the [src] answers and my head pounds for a moment before my vision clears. You are the avatar of [src], and my power is LIMITLESS! And it's all yours. You need to make sure no one can take it from you! No one must know, first!</span>", "<span class='sinister'>I hear a demonic hum, this can't be good!</span>")
			charges--
			insisting = 0
			...
			*/
			...
			to_chat(user, "<span class='sinister'>I have a very bad feeling about this!</span>")
