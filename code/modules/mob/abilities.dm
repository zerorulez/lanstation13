...
/client/proc/test_ability()
	...
	if(!mob || !istype(mob,/mob/living) || mob.stat)
		to_chat(src, "<span class='warning'>I must be corporeal and alive to do that.</span>")
		return 0

	//Handcuff check.
	if(mob.restrained())
		to_chat(src, "<span class='warning'>I cannot do this while restrained.</span>")
		return 0

	if(istype(mob,/mob/living/carbon))
		var/mob/living/carbon/M = mob
		if(M.handcuffed)
			to_chat(src, "<span class='warning'>I cannot do this while cuffed.</span>")
			return 0

	to_chat(src, "<span class='notice'>I perform an ability.</span>")
