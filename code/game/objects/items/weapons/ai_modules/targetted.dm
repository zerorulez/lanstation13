...
/obj/item/weapon/aiModule/targetted/upload(var/datum/ai_laws/laws, var/atom/target, var/mob/sender)
	...
	if (ismob(target) && is_special_character(target) && priority == LAW_ZERO)
		to_chat(target, "[sender.real_name] attempted to modify my zeroth law.")// And lets them know that someone tried. --NeoFite

		to_chat(target, "It would be in my best interest to play along with [sender.real_name] that [law]")
