	...
	Topic(href, href_list)
		if(..())
			...
			if(href_list["inject1"])
				...
				var/warning = copytext(sanitize(input(usr,"Message:","Enter my message here!","")),1,MAX_MESSAGE_LEN)
				if(!warning)
					return
				var/obj/item/weapon/implant/I = locate(href_list["warn"])
				if((I)&&(I.imp_in))
					var/mob/living/carbon/R = I.imp_in
					to_chat(R, "<span class='good'>I hear a voice in my head saying: '[warning]'</span>")
