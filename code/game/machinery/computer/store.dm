	...
	switch(href_list["choice"])
		if ("buy")
			...
			if(!centcomm_store.PlaceOrder(usr,itemID,src))
				to_chat(usr, "\icon[src]<span class='warning'>Unable to charge my account.</span>")
			else
				to_chat(usr, "\icon[src]<span class='notice'>Transaction complete! Enjoy my product.</span>")
