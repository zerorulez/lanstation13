	...
	switch(choice)
		...
		if("Rename")
			...
			if(length(new_name) > 0)
				...
				to_chat(usr, "I must write a name.")
...
/mob/living/silicon/ai/proc/ai_call_shuttle()
	if(src.stat == 2)
		to_chat(src, "I can't call the shuttle because you are dead!")
		return
...
/mob/living/silicon/ai/proc/ai_cancel_call()
	...
	if(src.stat == 2)
		to_chat(src, "I can't send the shuttle back because you are dead!")
		return
	if(istype(usr,/mob/living/silicon/ai))
		var/mob/living/silicon/ai/AI = src
		if(AI.control_disabled)
