...
/client/MouseDrop(src_object,over_object,src_location,over_location,src_control,over_control,params)
	...
	switch(buildmode)
		if(1 to 2)
			...
			if(fillturfs.len)
				if(alert("You are about to do a fill operation spanning [fillturfs.len] tiles, are you sure?","Panic","Yes","No") == "Yes")
		...
		if(3)
			var/list/fillturfs = block(start,end)
			if(fillturfs.len)
				if(alert("You are about to do a mass edit operation spanning [fillturfs.len] tiles, are you sure?","Panic","Yes","No") == "Yes")
					if(fillturfs.len > 150)
