...
//		to_chat(user, "<span class='notice'>You start to [panel_open ? "close" : "open"] the camera's panel.</span>")
	...
	// Upgrades!
		...
		if (is_type_in_list(W, assembly.upgrades))
			...
			return
		if (!panel_open)
			to_chat(user, "I can't reach into the camera's circuitry while the maintenance panel is closed.")
			return
		/*if (!wires.CanDeconstruct())
			to_chat(user, "I can't reach into the camera's circuitry with the wires on the way.")
			return*/
		if (istype(W, /obj/item/stack))
			...
			if(!user.drop_item(W, src))
				return
			assembly.upgrades += W
		to_chat(user, "I attach the [W] into the camera's inner circuits.")
		update_upgrades()
		update_icon()
		update_hear()
		cameranet.updateVisibility(src, 0)
	...
	// Taking out upgrades
		...
		if (assembly.upgrades.len)
			...
			if(U)
				to_chat(user, "I unattach \the [U] from the camera.")
	...
	// OTHER
		...
		to_chat(U, "I hold \a [W] up to the camera ...")
