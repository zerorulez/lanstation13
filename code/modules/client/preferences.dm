	...
	switch(href_list["task"])
		...
		if("input")
			switch(href_list["preference"])
				...
				if("s_tone")
					if(species == "Human")
						...
						if(skin_c)
							...
							switch(s_tone)
								if(6)
									to_chat(user,"My vox will now be emerald.")
								if(5)
									to_chat(user,"My vox will now be azure.")
								if(4)
									to_chat(user,"My vox will now be light green.")
								if(3)
									to_chat(user,"My vox will now be gray.")
								if(2)
									to_chat(user,"My vox will now be brown.")
								else
									to_chat(user,"My vox will now be green.")
					else
						to_chat(user,"My species doesn't have different skin tones. Yet?")
						return
				...
				if("flavor_text")
					var/msg = input(usr,"Texto que aparecerá quando os outros jogadores examinarem seu personagem. Use para falar sobre características físicas, sotaques ou o que mais você quer que saibam sobre seu personagem.","Flavor Text",html_decode(flavor_text)) as message
					if(msg != null)
