...
/obj/machinery/rust_fuel_compressor/attackby(obj/item/W, mob/user)
	...
	if (iscrowbar(W))
		...
		if(opened)
			...
			if(src.allowed(usr))
				locked = !locked
				to_chat(user, "I [ locked ? "lock" : "unlock"] the compressor interface.")
				update_icon()
			else
		...
		if(opened)
			...
			if (do_after(user, src,6))
				if(prob(50))
					emagged = 1
					locked = 0
					to_chat(user, "I emag the port interface.")
				else
					to_chat(user, "I fail to [ locked ? "unlock" : "lock"] the compressor interface.")
		return
		...
		to_chat(user, "I try to insert the circuitboard into the frame...")
		playsound(get_turf(src), 'sound/items/Deconstruct.ogg', 50, 1)
		if(do_after(user, src, 10))
			has_electronics &= 1
			to_chat(user, "I place the circuitboard inside the frame.")
			qdel(W)
		return
