...
/obj/item/device/modkit/afterattack(obj/O, mob/user as mob)
	...
	for(var/i=1;i<=original.len;i++)
		...
		if(istype(O,original_type))
			...
			j = i
	if(!to_type)
		to_chat(user, "<span class='warning'>I cannot modify \the [O] with this kit.</span>")
		return
	if(parts_left <= 0)
	...
	user.visible_message("<span class='warning'>[user] opens \the [src] and modifies \the [O] into \the [N].</span>","<span class='warning'>I open \the [src] and modify \the [O] into \the [N].</span>")
	qdel(O)
...
/obj/item/device/modkit/spur_parts
	...
	desc = "I can identify what looks like a gun barrel and various other miscellaneous parts. Clearly these must have some use..."
