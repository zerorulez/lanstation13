...
/obj/item/weapon/circuitboard/attackby(obj/item/I as obj, mob/user as mob)
	if(issolder(I))
		...
		if(WT.remove_fuel(1,user))
			...
			to_chat(user, "<span class='notice'>I melt away the circuitry, leaving behind a blank.</span>")
...
/obj/item/weapon/circuitboard/proc/solder_improve(mob/user as mob)
	to_chat(user, "<span class='warning'>I fiddle with a few random fuses but can't find a routing that doesn't short the board.</span>")
	return

/obj/item/weapon/circuitboard/supplycomp/solder_improve(mob/user as mob)
	to_chat(user, "<span class='notice'>I [contraband_enabled ? "" : "un"]connect the mysterious fuse.</span>")
...
/obj/item/weapon/circuitboard/security/solder_improve(mob/user as mob)
	...
	if(istype(src,/obj/item/weapon/circuitboard/security/engineering))
		...
		to_chat(user, "<span class='notice'>I locate a short that makes the feed circuitry more elegant.</span>")
	...
	switch(state)
		...
		if(2)
			...
			if(istype(P, /obj/item/stack/cable_coil))
				...
				if (C.amount < 5)
					to_chat(user, "<span class='warning'>I need at least 5 lengths of cable coil for this!</span>")
					return 1
		...
		if(3)
			...
			if(istype(P, /obj/item/stack/sheet/glass/glass))
				...
				if (G.amount < 2)
					to_chat(user, "<span class='warning'>I need at least 2 sheets of glass for this!</span>")
					return 1
