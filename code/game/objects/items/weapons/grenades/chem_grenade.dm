...
//				detonator.loc=src.loc
	...
	if(stage > 1 && !active && clown_check(user))
		to_chat(user, "<span class='attack'>I prime \the [name]!</span>")
...
/obj/item/weapon/grenade/chem_grenade/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(istype(W,/obj/item/device/assembly_holder) && (!stage || stage==1) && path != 2)
		...
		to_chat(user, "<span class='notice'>I tightly coil the wire around the metal casing.</span>")
...
//					to_chat(user, "<span class='warning'>I need to add at least one beaker before locking the assembly.</span>")
				to_chat(user, "<span class='warning'>I lock the empty assembly.</span>")
				name = "fake grenade"
			playsound(get_turf(src), 'sound/items/Screwdriver.ogg', 25, -3)
		...
		to_chat(user, "I begin pressing \the [W] into \the [src].")
		if(do_after(user, src, 30))
			to_chat(user, "I poke a hole in \the [src].")
			if(src.loc == user)
...
obj/item/weapon/grenade/chem_grenade/exgrenade/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(istype(W,/obj/item/device/assembly_holder) && (!stage || stage==1) && path != 2)
		...
		if(!det.secured)
			...
			return
		path = 1
		to_chat(user, "<span class='notice'>I insert [W] into the grenade.</span>")
		playsound(get_turf(src), 'sound/items/Screwdriver.ogg', 25, -3)
		user.remove_from_mob(det)
		det.forceMove(src)
		detonator = det
		...
		if(stage == 1)
			...
			if(active && prob(95))
				to_chat(user, "<span class='attack'>I trigger the assembly!</span>")
				prime()
				return
			else
				to_chat(user, "<span class='notice'>I unlock the assembly.</span>")
		...
		if(beakers.len == 3)
			...
			if (istype(W,/obj/item/slime_extract))
				if (inserted_cores > 1)
					to_chat(user, "<span class='warning'>I cannot fit more than two slime cores in this grenade.</span>")
				else
					if(user.drop_item(W, src))
						to_chat(user, "<span class='notice'>I add \the [W] to the assembly.</span>")
						beakers += W
