...
/obj/structure/bed/chair/vehicle/wheelchair/attackby(obj/item/weapon/W, mob/user)
	...
	if(istype(W, /obj/item/weapon/gun_barrel))
		to_chat(user, "I place \the [W] on \the [src].")
...
/obj/structure/bed/chair/vehicle/wheelchair/can_buckle(mob/M, mob/user)
	if(!Adjacent(user) || (!ishuman(user) && !isalien(user) && !ismonkey(user)) || user.restrained() || user.stat || user.locked_to || destroyed || occupant) //Same as vehicle/can_buckle, minus check for user.lying as well as allowing monkey and ayliens
		return 0
	return 1
