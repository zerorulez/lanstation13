...
/obj/item/weapon/paper/solar
	name = "paper - 'Going green with Greencorps! Instrunctions on setting up my own solar array.'"
	info = "<h1>Welcome</h1><p>We at Greencorps we love the environment, and space. With this package you will help mother nature and produce energy without using fossil fuel or plasma! The Singularity Engine is dangerous while solar energy is safe, which is why it's better. Now here is how you setup my own solar array.</p><p>I can make a solar panel by wrenching the solar assembly onto a cable node. Add a glass panel, reinforced or regular glass will do, which will finish the construction of my solar panel. It's that easy!.</p><p>Now after setting up 19 more of these solar panels you will want to create a solar tracker to keep track of mother nature's gift, the sun. These are the same steps as before except you insert the tracker equipment circuit into the assembly before performing the final step of adding the glass. You now have a tracker! Now the last step is to add a computer to calculate the sun's movements and to send commands to the solar panels to change direction with the sun. Setting up the solar computer is the same as setting up any computer, so you should have no trouble in doing that. You do need to put a wire node under the computer, and the wire needs to be connected to the tracker.</p><p>Congratulations, you should have a working solar array. If you are having trouble, here are some tips. Make sure all solar equipment are on a cable node, even the computer. You can always deconstruct my creations if you make a mistake.</p><p>That's all to it, be safe, be green!</p>"
...
/obj/machinery/power/solar_assembly/attackby(var/obj/item/weapon/W, var/mob/user)
	if(!anchored && isturf(loc))
		if(iswrench(W))
			...
			"<span class='notice'>I wrench [src] down.</span>")
		...
		if(iswrench(W))
			...
			"<span class='notice'>I unwrench [src] from the ground.</span>")
			playsound(get_turf(src), 'sound/items/Ratchet.ogg', 75, 1)
			return 1
		...
		if(istype(W, /obj/item/stack/sheet/glass))
			...
			if(S.amount >= 2)
				...
				"<span class='notice'>I carefully add glass to [src].</span>")
				if(tracker)
					new /obj/machinery/power/solar/panel/tracker(get_turf(src), src)
				else
					new /obj/machinery/power/solar/panel(get_turf(src), src)
			else
				to_chat(user, "<span class='notice'>I lack enough [W.name] to finish the solar.</span>")
			return 1
	...
	if(!tracker)
		if(istype(W, /obj/item/weapon/tracker_electronics))
			if(user.drop_item(W))
				...
				"<span class='notice'>I insert the electronics into [src].</span>")
				return 1
	else
		if(iscrowbar(W))
			...
			"<span class='notice'>I take the electronics out of [src].</span>")
			return 1
	..()
