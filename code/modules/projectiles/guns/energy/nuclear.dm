...
/obj/item/weapon/gun/energy/gun/nuclear/proc/failcheck()
	...
	if (prob(src.reliability))
		return 1 //No failure
	if (prob(src.reliability))
		for (var/mob/living/M in range(0,src)) //Only a minor failure, enjoy my radiation if you're in the same tile or carrying it
			if (src in M.contents)
				to_chat(M, "<span class='warning'>My gun feels pleasantly warm for a moment.</span>")
			else
		...
		for (var/mob/living/M in range(rand(1,4),src)) //Big failure, TIME FOR RADIATION BITCHES
			if (src in M.contents)
				to_chat(M, "<span class='warning'>My gun's reactor overloads!</span>")
