...
/obj/item/projectile/rocket/to_bump(var/atom/A)
	explosion(A, 1, 3, 5, 8) //RPGs pack a serious punch and will cause massive structural damage in my average room, but won't punch through reinforced walls
	if(!gcDestroyed)
		qdel(src)
...
/obj/item/projectile/nikita/to_bumped(var/atom/A)
	if(emagged && (A == mob))
		return
	detonate(A)
...
/obj/item/projectile/nikita/proc/detonate(var/atom/A)
	explosion(A, 1, 3, 5, 8) //Nikita rockets pack a serious punch and will cause massive structural damage in my average room, but won't punch through reinforced walls
	if(!gcDestroyed)
		qdel(src)
