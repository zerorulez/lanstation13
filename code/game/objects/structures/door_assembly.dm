...
/obj/structure/door_assembly/attackby(obj/item/W as obj, mob/user as mob)
	...
	if (istype(W, /obj/item/weapon/weldingtool) && ( (istext(glass)) || (glass == 1) || (!anchored) ))
		...
		if(anchored)
			user.visible_message("[user] unsecures the airlock assembly from the floor.", "I start to unsecure the airlock assembly from the floor.")
		else
			user.visible_message("[user] secures the airlock assembly to the floor.", "I start to secure the airlock assembly to the floor.")
		...
		user.visible_message("[user] removes the electronics from the airlock assembly.", "I start to install electronics into the airlock assembly.")
			...
			if (S.amount>=1)
				...
				if(istype(S, /obj/item/stack/sheet/glass/rglass))
					...
					if(S.amount>=2)
						...
						user.visible_message("[user] adds [S.name] to the airlock assembly.", "I start to install [S.name] into the airlock assembly.")
						if(do_after(user, src, 40))
							to_chat(user, "<span class='notice'>I installed [M] plating into the airlock assembly!</span>")
							S.use(2)
							glass = "[M]"
				busy = 0
