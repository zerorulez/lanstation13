	...
	if(istype(loc, /mob/living))
		...
		for(var/mob/dead/observer/O in get_active_candidates(ROLE_ALIEN,poll="[affected_mob] has been infected by \a [src]!"))
			if(O.client && O.client.desires_role(ROLE_ALIEN))
				if(check_observer(O))
					to_chat(O, "<span class=\"recruit\">I have automatically been signed up for \a [src]. (<a href='?src=\ref[O];jump=\ref[src]'>Teleport</a> | <a href='?src=\ref[src];signup=\ref[O]'>Retract</a>)</span>")
					ghost_volunteers += O
		spawn(0)
...
/obj/item/alien_embryo/proc/volunteer(var/mob/dead/observer/O)
	...
	if(O in ghost_volunteers)
		to_chat(O, "<span class='notice'>I will no longer be considered for this [src]. Click again to volunteer.</span>")
		ghost_volunteers.Remove(O)
		return
	if(!check_observer(O))
		to_chat(O, "<span class='warning'>I cannot be \a [src] in my current condition.</span>")
		return
	to_chat(O, "<span class='notice'>I have been added to the list of ghosts that may become this [src].  Click again to unvolunteer.</span>")
	...
	switch(stage)
		...
		if(4)
			...
			if(prob(1))
				affected_mob.audible_cough()
			if(prob(2))
				to_chat(affected_mob, "<span class='warning'>My muscles ache.</span>")
				if(prob(20))
					affected_mob.take_organ_damage(1)
			if(prob(2))
				to_chat(affected_mob, "<span class='warning'>My stomach hurts.</span>")
				if(prob(20))
					affected_mob.AdjustPlasma(1)
					affected_mob.updatehealth()
		if(5)
			to_chat(affected_mob, "<span class='danger'>I feel something tearing its way out of my stomach...</span>")
