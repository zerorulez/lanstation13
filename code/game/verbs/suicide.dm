...
/mob/living/carbon/human/verb/suicide()
	set hidden = 1
	if(!ticker)
		to_chat(src, "<span class='warning'>I can't commit suicide before the game starts!</span>")
		return

	if(stat == DEAD)
		to_chat(src, "<span class='warning'>I am already dead!</span>")
		return
	...
	if(B && B.controlling) //Borer
		to_chat(src, "<span class='warning'>I cannot commit suicide, my host is clinging to life enough to resist it.</span>")
		return
	...
	if(!permitted)
		...
		to_chat(src, "<span class='warning'>My masters and the gods won't let you do that without a proper reason.</span>")
		return

	if(suiciding)
		to_chat(src, "<span class='warning'>I am already committing suicide! Be patient!</span>")
		return
	...
	if(confirm == "Yes")
		if(!canmove || restrained()) //Just while I finish up the new 'fun' suiciding verb. This is to prevent metagaming via suicide
			to_chat(src, "<span class='warning'>I can't commit suicide whilst restrained!</span>")
			return
		suiciding = 1
...
/mob/living/carbon/monkey/verb/suicide()
	...
	var/mob/living/simple_animal/borer/B=has_brain_worms()
	if (B && B.controlling) // Borer
		to_chat(src, "My can't suicide while controlling my host, you dick.")
		return
	...
	if(confirm == "Yes")
		if(!canmove || restrained())
			to_chat(src, "I can't commit suicide whilst restrained! ((You can type Ghost instead however.))")
			return
		suiciding = 1
