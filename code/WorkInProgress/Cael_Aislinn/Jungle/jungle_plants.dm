...
/obj/structure/bush/attackby(var/obj/I as obj, var/mob/user as mob)
	...
	if(istype(I, /obj/item/weapon/hatchet) && !stump)
		if(indestructable)
			...
			to_chat(user, "<span class='warning'>I flail away at the undergrowth, but it's too thick here.</span>")
		else
			user.visible_message("<span class='danger'><b>[user] begins clearing away [src].</b>","<span class='warning'>I begin clearing away [src].</span></span>")
			spawn(rand(15,30))
				if(get_dist(user,src) < 2)
					to_chat(user, "<span class='notice'>I clear away [src].</span>")
...
/obj/structure/jungle_plant/attack_hand(var/mob/user as mob)
	if(fruits_left > 0)
		fruits_left--
		to_chat(user, "<span class='notice'>I pick a fruit off [src].</span>")
