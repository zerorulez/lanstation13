...
/obj/machinery/telepad/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(isscrewdriver(W))
		if(opened)
			...
			to_chat(user, "<span class = 'caution'>I secure the access port on \the [src].</span>")
			opened = 0
		else
			playsound(src, 'sound/items/Screwdriver.ogg', 50, 1)
			to_chat(user, "<span class = 'caution'>I open \the [src]'s access port.</span>")
			opened = 1
	if(istype(W, /obj/item/bluespace_crystal) && opened)
		...
		to_chat(user, "<span class = 'caution'>I jam \the [W] into \the [src]'s booster coil.</span>")
		user.u_equip(W,1)
		W.forceMove(src)
		amplifier=W
		return
	if(iscrowbar(W) && opened && amplifier)
		to_chat(user, "<span class='notice'>I carefully pry \the [amplifier] from \the [src].</span>")
