...
/obj/item/clothing/description_accessories()
	if(accessories.len)
		. = list()
		for(var/obj/item/clothing/accessory/accessory in accessories)
			. += "<b>\icon[accessory] \a [accessory]</b>"
	...
	if(ishuman(M) && isliving(user))
		if(user.a_intent == I_HELP)
			...
			if(body_part)
				...
				user.visible_message("[user] places [src] against [M]'s [body_part] and listens attentively.", "I place [src] against [their] [body_part]. You [sound_strength] [sound].")
				return
	return ..(M,user)
