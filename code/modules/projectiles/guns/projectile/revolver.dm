/obj/item/weapon/gun/projectile/detective
	...
	verb/rename_gun()
		...
		if(src && input && !M.stat && in_range(src,M))
			name = input
			to_chat(M, "I name the gun [input]. Say hello to your new friend.")
			return 1
...
/obj/item/weapon/gun/projectile/russian/attack(atom/target as mob|obj|turf|area, mob/living/user as mob|obj)
	...
	if(isliving(target) && isliving(user) && target == user)
		if(mouthshoot)
			to_chat(user, "<span class='warning'>You are already doing that.</span>")
			return
		...
		if(affecting == LIMB_HEAD || affecting == "mouth")
			...
			playsound(user, fire_sound, 50, 1)
			user.visible_message("<span class='danger'>[user.name] fires \the [src]!</span>", "<span class='danger'>You fire \the [src]!</span>", "I hear a [istype(in_chamber, /obj/item/projectile/beam) ? "laser blast" : "gunshot"]!")
			if(!P.nodamage)
				affecting = LIMB_HEAD
