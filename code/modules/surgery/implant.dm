...
/datum/surgery_step/cavity/make_space/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"I start making some space inside [target]'s [get_cavity(affected)] cavity with \the [tool]." )
...
/datum/surgery_step/cavity/make_space/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='notice'>I make some space inside [target]'s [get_cavity(affected)] cavity with \the [tool].</span>" )
...
/datum/surgery_step/cavity/make_space/fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='warning'>My hand slips, scraping tissue inside [target]'s [affected.display_name] with \the [tool]!</span>")
...
/datum/surgery_step/cavity/close_space/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"I start mending [target]'s [get_cavity(affected)] cavity wall with \the [tool]." )
	target.custom_pain("The pain in my chest is living hell!",1)
	affected.cavity = 0
	..()
...
/datum/surgery_step/cavity/close_space/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='notice'>I mend [target]'s [get_cavity(affected)] cavity walls with \the [tool].</span>" )
...
/datum/surgery_step/cavity/place_item/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"I start putting \the [tool] inside [target]'s [get_cavity(affected)] cavity." )
	target.custom_pain("The pain in my chest is living hell!",1)
	..()
...
/datum/surgery_step/cavity/place_item/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='notice'>I put \the [tool] inside [target]'s [get_cavity(affected)] cavity.</span>" )
	if (tool.w_class > get_max_wclass(affected)/2 && prob(50))
		to_chat(user, "<span class='warning'>I tear some vessels trying to fit such big object in this cavity.")
		var/datum/wound/internal_bleeding/I = new (15)
		affected.wounds += I
		affected.owner.custom_pain("I feel something rip in my [affected.display_name]!", 1)
	user.drop_item()
...
/datum/surgery_step/cavity/implant_removal/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"I start poking around inside the incision on [target]'s [affected.display_name] with \the [tool]." )
	target.custom_pain("The pain in my chest is living hell!",1)
	..()
...
/datum/surgery_step/cavity/implant_removal/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	if (affected.implants.len)
		...
		if (prob(find_prob))
			...
			"<span class='notice'>I take [obj] out of incision on [target]'s [affected.display_name]s with \the [tool].</span>" )
		...
		user.visible_message("<span class='notice'>[user] takes something out of incision on [target]'s [affected.display_name] with \the [tool].</span>", \
		"<span class='notice'>I take something out of incision on [target]'s [affected.display_name]s with \the [tool].</span>" )
		...
		"<span class='notice'>I could not find anything inside [target]'s [affected.display_name].</span>" )
