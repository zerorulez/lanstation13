...
/mob/proc/handle_enthrall(mob/living/carbon/human/H as mob)
	...
	new_objective.explanation_text = "I have been Enthralled by [src.name], the vampire. Follow their every command."
...
/client/proc/vampire_bats()
	...
	set desc = "I summon a pair of space bats who attack nearby targets until they or their target is dead."
	var/datum/mind/M = usr.mind
	if(!M)
		return
...
/client/proc/vampire_jaunt()
	...
	set desc = "I become ethereal and can travel through walls for a short time, while leaving a scary bat behind."
