...
/datum/disease/robotic_transformation/stage_act()
	...
	switch(stage)
		if(2)
			if (prob(8))
				to_chat(affected_mob, "My joints feel stiff.")
		...
		if(3)
			if (prob(8))
				to_chat(affected_mob, "<span class='warning'>My joints feel very stiff.</span>")
				affected_mob.take_organ_damage(1)
			if (prob(8))
				affected_mob.say(pick("Beep, boop", "beep, beep!", "Boop boop!"))
			if (prob(10))
				to_chat(affected_mob, "My skin feels loose.")
		...
		if(4)
			if (prob(10))
				to_chat(affected_mob, "<span class='warning'>My skin feels very loose.</span>")
				affected_mob.take_organ_damage(8)
			if (prob(20))
				affected_mob.say(pick("beep, beep!", "Boop bop boop beep.", "kkkiiiill mmme", "I wwwaaannntt tttoo dddiiieeee..."))
			if (prob(8))
				to_chat(affected_mob, "<span class='warning'>I can feel... something...inside you.</span>")
		if(5)
			to_chat(affected_mob, "<span class='warning'>My skin feels as if it's about to burst off...</span>")
