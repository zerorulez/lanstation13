	...
	if ((usr.contents.Find(src) || (in_range(src, usr) && istype(src.loc, /turf))))
		...
		if (href_list["refresh"])
			...
			if (sr)
				...
				src.temp += "<B>I am at \[[sr.x-WORLD_X_OFFSET[sr.z]],[sr.y-WORLD_Y_OFFSET[sr.z]],[sr.z]\]</B> in orbital coordinates.<BR><BR><A href='byond://?src=\ref[src];refresh=1'>Refresh</A><BR>"
			else
