...
/obj/item/verbs/borer/attached_arm/bone_sword/verb/bone_sword()
	set category = "Alien"
	set name = "Bone Sword"
	set desc = "Expend chemicals constantly to form a large blade of bone for my host."
...
/mob/living/simple_animal/borer/proc/bone_sword()
	...
	if(channeling && !channeling_bone_sword)
		...
		to_chat(src, "I cease my efforts to sustain a blade of bone for my host.")
		channeling = 0
		...
		if(hostlimb == LIMB_RIGHT_ARM)
			if(host.get_held_item_by_index(GRASP_RIGHT_HAND))
				if(istype(host.get_held_item_by_index(GRASP_RIGHT_HAND), /obj/item/weapon/melee/bone_sword))
					to_chat(src, "<span class='warning'>My host already has a bone sword on this arm.</span>")
					qdel(S)
					return
		...
		to_chat(src, "I begin to focus my efforts on sustaining a blade of bone for my host.")
		channeling = 1
		channeling_bone_sword = 1
		host.visible_message("<span class='warning'>A blade of bone erupts from \the [host.name]'s [hostlimb == LIMB_RIGHT_ARM ? "right" : "left"] arm!</span>","<span class='warning'>A blade of bone erupts from my [hostlimb == LIMB_RIGHT_ARM ? "right" : "left"] arm!</span>")
		spawn()
			...
			channeling = 0
			channeling_bone_sword = 0
			host.visible_message("<span class='notice'>\The [host]'s bone sword crumbles into nothing.</span>","<span class='notice'>My bone sword crumbles into nothing.</span>")
			var/showmessage = 0
			if(chemicals < 5)
...
/obj/item/verbs/borer/attached_arm/bone_hammer/verb/bone_hammer()
	...
	set desc = "Expend chemicals constantly to form a large, heavy mass of bone on my host's arm."
...
/mob/living/simple_animal/borer/proc/bone_hammer()
	...
	if(channeling && !channeling_bone_hammer)
		...
		to_chat(src, "I cease my efforts to sustain a mass of bone for my host.")
		channeling = 0
		...
		if(hostlimb == LIMB_RIGHT_ARM)
			if(host.get_held_item_by_index(GRASP_RIGHT_HAND))
				if(istype(host.get_held_item_by_index(GRASP_RIGHT_HAND), /obj/item/weapon/melee/bone_hammer))
					to_chat(src, "<span class='warning'>My host already has a bone hammer on this arm.</span>")
					qdel(S)
					return
				host.drop_item(host.get_held_item_by_index(GRASP_RIGHT_HAND), force_drop = 1)
			host.put_in_r_hand(S)
			...
			if(host.get_held_item_by_index(GRASP_LEFT_HAND))
				...
				host.drop_item(host.get_held_item_by_index(GRASP_LEFT_HAND), force_drop = 1)
			host.put_in_l_hand(S)
		to_chat(src, "I begin to focus my efforts on sustaining a mass of bone for my host.")
		channeling = 1
		channeling_bone_hammer = 1
		host.visible_message("<span class='warning'>A mass of bone erupts from \the [host.name]'s [hostlimb == LIMB_RIGHT_ARM ? "right" : "left"] arm!</span>","<span class='warning'>A mass of bone erupts from my [hostlimb == LIMB_RIGHT_ARM ? "right" : "left"] arm!</span>")
		spawn()
			var/time_spent_channeling = 0
			...
			channeling = 0
			channeling_bone_hammer = 0
			host.visible_message("<span class='notice'>\The [host]'s bone hammer crumbles into nothing.</span>","<span class='notice'>My bone hammer crumbles into nothing.</span>")
			var/showmessage = 0
			if(chemicals < 10)
				to_chat(src, "<span class='warning'>I lose consciousness as the last of my chemicals are expended.</span>")
			else
...
/obj/item/verbs/borer/attached_arm/bone_shield/verb/bone_shield()
	...
	set desc = "Expend chemicals constantly to form a large shield of bone for my host."
...
/mob/living/simple_animal/borer/proc/bone_shield()
	...
	if(channeling && !channeling_bone_shield)
		...
		to_chat(src, "I cease my efforts to sustain a shield of bone for my host.")
		channeling = 0
		...
		if(hostlimb == LIMB_RIGHT_ARM)
			if(host.get_held_item_by_index(GRASP_RIGHT_HAND))
				if(istype(host.get_held_item_by_index(GRASP_RIGHT_HAND), /obj/item/weapon/shield/riot/bone))
					to_chat(src, "<span class='warning'>My host already has a bone shield on this arm.</span>")
					qdel(S)
					return
				host.drop_item(host.get_held_item_by_index(GRASP_RIGHT_HAND), force_drop = 1)
			host.put_in_r_hand(S)
			...
			if(host.get_held_item_by_index(GRASP_LEFT_HAND))
				...
				host.drop_item(host.get_held_item_by_index(GRASP_LEFT_HAND), force_drop = 1)
			host.put_in_l_hand(S)
		to_chat(src, "I begin to focus my efforts on sustaining a shield of bone for my host.")
		channeling = 1
		channeling_bone_shield = 1
		host.visible_message("<span class='warning'>A shield of bone erupts from \the [host.name]'s [hostlimb == LIMB_RIGHT_ARM ? "right" : "left"] arm!</span>","<span class='warning'>A shield of bone erupts from my [hostlimb == LIMB_RIGHT_ARM ? "right" : "left"] arm!</span>")
		spawn()
			var/time_spent_channeling = 0
			...
			channeling = 0
			channeling_bone_shield = 0
			host.visible_message("<span class='notice'>\The [host]'s bone shield crumbles into nothing.</span>","<span class='notice'>My bone shield crumbles into nothing.</span>")
			var/showmessage = 0
			if(chemicals < 3)
				to_chat(src, "<span class='warning'>I lose consciousness as the last of my chemicals are expended.</span>")
			else
...
/obj/item/verbs/borer/attached_arm/bone_cocoon/verb/bone_cocoon()
	...
	set desc = "Expend chemicals constantly to form a large protective cocoon of bone around my host."
...
/mob/living/simple_animal/borer/proc/bone_cocoon()
	...
	if(channeling && !channeling_bone_cocoon)
		...
		to_chat(src, "I begin to focus my efforts on sustaining a cocoon of bone for my host.")
		channeling = 1
		channeling_bone_cocoon = 1
		host.visible_message("<span class='warning'>A cocoon of bone sprouts from \the [host.name]'s [hostlimb == LIMB_RIGHT_ARM ? "right" : "left"] arm and envelops \him!</span>","<span class='warning'>A cocoon of bone erupts from my [hostlimb == LIMB_RIGHT_ARM ? "right" : "left"] arm and envelops you!</span>")
		host.forceMove(S)
		spawn()
			var/time_spent_channeling = 0
...
/mob/living/simple_animal/borer/proc/em_pulse()
	...
	if(chemicals < 100)
		to_chat(src, "<span class='warning'>I need at least 100 chemicals to do this.</span>")
		return
	else
		chemicals -= 100
...
/obj/item/verbs/borer/attached_arm/repair_bone/verb/repair_bone()
	...
	set desc = "Expend chemicals to repair bones in my host's arm."
...
/mob/living/simple_animal/borer/proc/repair_bone()
	...
	if(!istype(host, /mob/living/carbon/human))
		to_chat(src, "<span class='warning'>I can't seem to repair my host's strange biology.</span>")
		return
	if(chemicals < 30)
		to_chat(src, "<span class='warning'>I don't have enough chemicals stored to do this.</span>")
		return
	...
	for(var/datum/organ/external/O in current_limb.children)
		if(O.is_broken())
			...
			to_chat(src, "<span class='notice'>I've repaired the bones in my host's [hostlimb == LIMB_RIGHT_ARM ? "right" : "left"] hand.</span>")
			to_chat(host, "<span class='notice'>I feel the bones in my [hostlimb == LIMB_RIGHT_ARM ? "right" : "left"] hand mend together.</span>")
			chemicals -= 30
			has_healed_hand = 1
	if(current_limb.is_broken())
		if(chemicals < 50)
			if(has_healed_hand)
				to_chat(src, "<span class='warning'>I don't have enough chemicals left to heal my host's [hostlimb == LIMB_RIGHT_ARM ? "right" : "left"] arm.</span>")
				return
			else
				to_chat(src, "<span class='warning'>I must have at least 50 chemicals stored to heal a broken arm.</span>")
				return
		...
		to_chat(src, "<span class='notice'>I've repaired the bones in my host's [hostlimb == LIMB_RIGHT_ARM ? "right" : "left"] arm.</span>")
		to_chat(host, "<span class='notice'>I feel the bones in my [hostlimb == LIMB_RIGHT_ARM ? "right" : "left"] arm mend together.</span>")
		chemicals -= 50
	else
		if(!has_healed_hand)
			to_chat(src, "<span class='notice'>None of the bones in my host's [hostlimb == LIMB_RIGHT_ARM ? "right" : "left"] arm or hand are broken.</span>")