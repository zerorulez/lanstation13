		...
		if ((src.loc && (get_dist(src, usr) < 2 || src.loc == usr.loc)))
			...
			if ( (direct - 1) & direct)
				...
				if(Step_1 && Step_2)
					...
					/*
						More info:
							If you're trying to click an item in the north-east of my mob, the above section of code will first check if tehre's a tile to the north or you and to the east of you
							These two tiles are Step_1 and Step_2. After this, a new dummy object is created on my location. It then tries to move to Step_1, If it succeeds, objects on the turf you're on and
							...
							CheckExit() and CanPass() are use to determine this. The dummy object is then moved back to my location and it tries to move to Step_2. Same checks are performed here.
