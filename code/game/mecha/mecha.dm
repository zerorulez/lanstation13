...
/obj/mecha/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if(istype(W, /obj/item/mecha_parts/mecha_equipment))
		...
		spawn()
			if(E.can_attach(src))
				...
				to_chat(user, "I was unable to attach [W] to [src]")
		return
	if(istype(W, /obj/item/weapon/card/id)||istype(W, /obj/item/device/pda))
		...
		if(state==STATE_BOLTSOPENED)
			if(!src.tracking)
				if(user.drop_item(W, src))
					to_chat(user, "I install the tracking beacon and safeties.")
					src.tracking = W
		...
		if(state==STATE_BOLTSOPENED)
			if(!src.electropack)
				if(user.drop_item(W, src))
					to_chat(user, "I rig the electropack to the cockpit.")
					src.electropack = W
...
//	to_chat(user, "Installing MMI, please stand by.")
	...
	if(enter_after(40,user))
		...
		to_chat(user, "I stop inserting the MMI.")
	return 0
...
/obj/mecha/proc/go_out(var/exit = loc)
	...
	/*
		...
		//Can not have a pressure delta that would cause environment pressure > tank pressure
			...
			to_chat(occupant, "I was blown out of the mech!")
	*/
...
/obj/mecha/Topic(href, href_list)
	...
	if(href_list["select_equip"])
		if(usr != src.occupant)
			return
		var/obj/item/mecha_parts/mecha_equipment/equip = t_filter.getObj("select_equip")
		if(equip)
			src.selected = equip
	...
	if(href_list["rfreq"])
		if(usr != src.occupant)
			return
		var/new_frequency = (radio.frequency + t_filter.getNum("rfreq"))
	...
	if(href_list["req_access"] && add_req_access)
		if(!in_range(src, usr))
			return
		output_access_dialog(t_filter.getObj("id_card"),t_filter.getMob("user"))
		return
	if(href_list["maint_access"] && maint_access)
		if(!in_range(src, usr))
			return
		var/mob/user = t_filter.getMob("user")
		if(user)
			if(state==STATE_BOLTSHIDDEN)
				...
				to_chat(user, "I can't toggle maintenance mode with the securing bolts unfastened.")
			output_maintenance_dialog(t_filter.getObj("id_card"),user)
		return
	...
	if(href_list["add_req_access"] && add_req_access && t_filter.getObj("id_card"))
		if(!in_range(src, usr))
			return
		operation_req_access += t_filter.getNum("add_req_access")
		output_access_dialog(t_filter.getObj("id_card"),t_filter.getMob("user"))
		return
	if(href_list["del_req_access"] && add_req_access && t_filter.getObj("id_card"))
		if(!in_range(src, usr))
			return
		operation_req_access -= t_filter.getNum("del_req_access")
		output_access_dialog(t_filter.getObj("id_card"),t_filter.getMob("user"))
		return
	...
	if(href_list["debug"])
		if(href_list["set_i_dam"])
			setInternalDamage(t_filter.getNum("set_i_dam"))
		if(href_list["clear_i_dam"])
			clearInternalDamage(t_filter.getNum("clear_i_dam"))
		return
	*/
